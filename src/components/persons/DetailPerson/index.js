                    ...
                    <div className="row detail-film" key={personDetail && personDetail.id}>
                        ...
                        <img src={personDetail && personDetail.birthday !== null ? `http://image.tmdb.org/t/p/w500${personDetail && personDetail.profile_path}` : "https://via.placeholder.com/300x400/2C2F33/FFFFFF/png?text=Image+non+disponible"} className="card-img-top" alt={`Poster de ${personDetail && personDetail.name}`} />
                            ...
                            {personDetail && personDetail.length > 0 && (<p className="film-detail film-detail-duree">
                                ...
                                <span>
                                    {personDetail && personDetail.also_known_as.map((name) => <p>{name}</p>)}
                                </span>
                            </p>)}
                            ...
                            {personDetail.deathday && (
                                <p className="film-detail">
                                    Date de décès
                                    <span>
                                        {moment(personDetail && personDetail.deathday).format('DD/MM/YYYY')}
                                    </span>
                                </p>
                            )}
                            {personDetail && personDetail.biography && (<p className="film-detail">
                                Biographie
                                <span>
                                    <div className="film-detail-keywords">{personDetail && personDetail.biography}</div>
                                </span>
                            </p>)}
                            {personDetail && personDetail.homepage && (<p className="film-detail">
                                Homepage
                                <span>
                                    <div className="film-detail-keywords"><a href={personDetail.homepage}>{personDetail && personDetail.homepage}</a></div>
                                </span>
                            </p>)}
                        </div>
                        <div className="col-xs-12 col-md-8">
                            <ul className="nav nav-tabs detail-film-videos" id="nav-tab" role="tablist">
                                <a className="nav-item nav-link active" onClick={toggleTabs} id="nav-credits-tab" data-toggle="tab" href="#nav-credits" role="tab" aria-controls="nav-credits" aria-selected="true">Crédits</a>
                                {photosPerson && photosPerson.length > 0 && <a className="nav-item nav-link" onClick={toggleTabs} id="nav-photos-tab" data-toggle="tab" href="#nav-photos" role="tab" aria-controls="nav-photos" aria-selected="false">Photos</a>}
                            </ul>
                            <div className="tab-content" id="nav-tabContent">
                                <div className="tab-pane fade show active" id="nav-credits" role="tabpanel" aria-labelledby="nav-credits-tab">
                                    <div className="card-body">
                                        <p className="film-detail">
                                            Casting
                                            <span>
                                                <div className="film-detail-cast film-detail-cast-photos scrolling-wrapper">
                                                    {castCreditsPerson && castCreditsPerson.map((credit) => 
                                                        <div key={credit.id} className="card">
                                                            <Link href={`/film/${credit.id}`} to={`/film/${credit.id}`}><img src={credit.poster_path == null ? "https://via.placeholder.com/200x300/2C2F33/FFFFFF/png?text=Image+non+disponible" : `http://image.tmdb.org/t/p/w500${credit.poster_path}`} className="card-img-top" alt={credit.name} /></Link>
                                                            <br />
                                                            <p className="film-detail-cast-name">{!credit.title ? credit.name : credit.title}</p>
                                                            <p className="film-detail-cast-character">{credit.character === "" ? "-" : credit.character}</p>
                                                        </div>
                                                    )}
                                                </div>  
                                            </span>
                                        ...
                                        <p className="film-detail">
                                            ...
                                            <span>
                                                <div className="film-detail-cast film-detail-cast-photos scrolling-wrapper">
                                                    {crewCreditsPerson && crewCreditsPerson.map((crew) => 
                                                        <div key={crew.id} className="card">
                                                            <Link href={`/film/${crew.id}`} to={`/film/${crew.id}`}><img src={crew.poster_path == null ? "https://via.placeholder.com/200x300/2C2F33/FFFFFF/png?text=Image+non+disponible" : `http://image.tmdb.org/t/p/w500${crew.poster_path}`} className="card-img-top" alt={crew.name} /></Link>
                                                            <br />
                                                            <p className="film-detail-cast-name">{crew.title === "" ? crew.name : crew.title}</p>
                                ...
                                {photosPerson && photosPerson.length > 0 && (
                                    <div className="tab-pane fade" id="nav-photos" role="tabpanel" aria-labelledby="nav-photos-tab">
                                        <div className="film-detail-photos">
                                            {photosPerson && photosPerson.map((photo, index) => <img key={index} src={`http://image.tmdb.org/t/p/w500${photo.file_path}`} />)}
                                        </div>
                                    </div>
                                )}
                            </div>
