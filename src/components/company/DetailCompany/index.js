    ...
    return (
        <>
            ...
            <div className="container content">
                {pending ? <Spinner /> : 
                    <div className="row detail-film" key={companyDetail && companyDetail.id}>
                        <div className="col-xs-12 detail-film-poster">
                            <img src={`http://image.tmdb.org/t/p/w500${companyDetail && companyDetail.logo_path}`} style={{ padding: "10px" }} className="card-img-top" alt={`Poster de ${companyDetail && companyDetail.name}`} />
                            <h2>{companyDetail && companyDetail.name}</h2>
                            {companyDetail && companyDetail.origin_country && (<p className="film-detail film-detail-duree">
                                Origine
                                <span>
                                    <Flag code={`${companyDetail.origin_country}`} key={companyDetail.name} className="film-production-flag" />
                                </span>
                            </p>)}
                            {companyDetail && companyDetail.description && (<p className="film-detail">
                                Description
                                <span>
                                    <div className="film-detail-keywords">{companyDetail && companyDetail.description}</div>
                                </span>
                            </p>)}
                            <p className="film-detail">
                                Siège social
                                <span>
                                {companyDetail && companyDetail.headquarters}
                                </span>
                            </p>
                            {companyDetail && companyDetail.parent_company && (<p className="film-detail">
                                Maison mère
                                <span>
                                    <div className="film-detail-keywords">{companyDetail && companyDetail.parent_company}</div>
                                </span>
                            </p>)}
                            {companyDetail && companyDetail.homepage && (<p className="film-detail">
                                Homepage
                                <span>
                                    <div className="film-detail-keywords"><a href={companyDetail.homepage}>{companyDetail && companyDetail.homepage}</a></div>
                                </span>
                            </p>)}
                        </div>
                    </div>
